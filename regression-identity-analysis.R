# Analyse results from an Identity TC regression test

# Run in rstudio with:

# source("~/R_analysis/regression-identity-analysis.R")
...
nmeta_version <- readline("nmeta-identity or nmeta2-identity?")
...
files_lg <- fx_build_file_data("lg1.example.com-iperf_result.txt", files_dir_2)
files_pc <- fx_build_file_data("pc1.example.com-iperf_result.txt", files_dir_2)
...
for (i in 1:length(files_list_lg)) {
    ...
    dir_path <- unname(files_lg$dir_path[i])
    df_tmp <- data.frame(i)
    colnames(df_tmp)[1] = "Test Number"
    # Add in the 'y' column(s):
    df_tmp[2] <- files_list_lg[[i]][,5]
    df_tmp[3] <- files_list_lg[[i]][,9]
